...
const addPatient = asyncHandler(async (req, res) => {
  const { firstName, lastName, nationalIdType, nationalId } = req.body;

  const nationalIdExist = await Patient.findOne({ nationalId: nationalId });

  if (nationalIdExist) {
    res.status(400);
    res.json({ message: 'National ID already exists' });
    return;
  }
