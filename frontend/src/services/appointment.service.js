  ...
  for (var i = 0; i < timeArr.length; i++) {
    ...
    while (new Date(_tempSlotStartTime).getTime() < new Date(_timeArrEndTime).getTime()) {
      ...
      if (new Date(_tempSlotStartTime).getTime() <= new Date(_timeArrEndTime).getTime()) {
        ...
        session.slots.push({
          timeSlotStart: new Date(_startSlot).toLocaleTimeString('es', {hour: 'numeric', minute: 'numeric', hour12: false}),
          timeSlotEnd: _endSlot.toLocaleTimeString('es', { hour: 'numeric', minute: 'numeric', hour12: false }),
